...
<JobIcon icon="TankRole"/>**4 Tanks** bring their assigned golem to its room and use Interject on the Metamorphosis if needed.

<JobIcon icon="HealerRole"/>**Healers** will keep their tank alive with their basic heals, and use Esuna to remove a deadly bleed on the two **Blue Golem Tanks**.

<JobIcon icon="DPSRole"/>**1-2 DPS** should handle the 2 uncolored Ruin Golems that spawn in the hallway. Simply keeping them out of the green room while killing them.
...
These golems will tether to the first person that attacks them. Holding their hate/aggro on that tethered player until they die or the person tethered dies. These will need to be kept away from the Green Golem since it will absorb the uncolored golems for a near lethal damage up.

Once all Bicolor Golems are dead these golems will start to spawn at a much more frequent rate. As well as spamming **Demolish** which is a massive AoE that applies a Thrice Come Ruin if allowed to finish. During this time the uncolored Ruin Golems should be crowd controlled using a Caster's **Sleep**, 
Healer's **Repose**, WHM's **Holy**, Tank's **Low Blow**, or a Melee's **Leg Sweep**.
...
#### Blue Golem
Blue Golems have the **Torrential Ruin** buff require the most attention from healers as they cast the **Core Combustion** tankbuster. This is medium sized AoE around the main tank that inflicts a incredibly lethal bleed (~50k per tick) to anyone hit by it. This bleed _must_ be cleansed with either **Esuna**, **Resistance Medikit**,
...
#### Green Golem
Green Golems have a buff called **Avaricious Ruin** which grants it the ability to consume/absord the uncolored Ruin Golems that spawn in the hallway. If any golems are absord, the Green golem will gain a potent damage up and being autoing the tank for near lethal damage. You can recover from an abosrbed golem. 
However, both heavy mitigation and healing is required to keep the tank alive. 
